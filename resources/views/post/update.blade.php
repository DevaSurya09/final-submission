...
<h1 style="text-align: center; font-weight: bold; font-size: 30px; margin-bottom: 20px;">Edit Post</h1>

@if (session()->has('sukses'))
    <p style="text-align: center; color: green; font-weight: bold;">{{ session('sukses') }}</p>
@elseif (session()->has('error'))
    <p style="text-align: center; color: red; font-weight: bold;">{{ session('error') }}</p>
@endif
...
<div style="max-width: 500px; margin: 0 auto; padding: 20px; border-radius: 10px; background-color: #f4f4f4;
    ...
    <form action="{{ route('post.update', $post->id) }}" method="post">
        ...
        <input type="text" name="title" id="title" value="{{ $post->title }}" style="width: 100%; padding: 10px;
                border-radius: 5px; border: 1px solid #ccc; margin-bottom: 10px;">
        @if ($errors->has('title'))
        ...
        <textarea name="description" id="description" rows="4" style="width: 100%; padding: 10px; border-radius: 5px;
                border: 1px solid #ccc; margin-bottom: 10px;">{{ $post->description }}</textarea>
        ...
        <input type="text" name="author" id="author" value="{{ $post->author }}" style="width: 100%; padding: 10px;
                border-radius: 5px; border: 1px solid #ccc; margin-bottom: 10px;">
